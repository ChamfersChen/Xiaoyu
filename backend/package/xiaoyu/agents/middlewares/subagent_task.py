...
from xiaoyu.agents.context import build_agent_input_context
from xiaoyu.repositories.agent_repository import SUB_AGENT_BACKEND_ID, AgentRepository
from xiaoyu.repositories.agent_run_repository import AgentRunRepository
from xiaoyu.repositories.user_repository import UserRepository
from xiaoyu.storage.postgres.manager import pg_manager
from xiaoyu.storage.postgres.models_business import Agent
from xiaoyu.utils.datetime_utils import utc_isoformat
from xiaoyu.utils.subagent_thread_utils import make_child_thread_id
...
TASK_TOOL_DESCRIPTION = """Launch a configured Xiaoyu subagent to handle an isolated task.
...
class XiaoyuSubAgentMiddleware(AgentMiddleware[Any, ContextT, ResponseT]):
...
async def create_subagent_task_middleware(parent_context) -> XiaoyuSubAgentMiddleware | None:
    selected_slugs = [
    ...
    if not subagents:
        return None
    return XiaoyuSubAgentMiddleware(parent_context=parent_context, subagents=subagents)
