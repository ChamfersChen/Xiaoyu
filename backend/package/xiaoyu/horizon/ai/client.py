...
from xiaoyu.horizon.models import AIConfig, AIProvider
        ...
        for i in range(len(self.configs)):
            try:
                ...
                if i < len(self.configs) - 1:
                    logger.warning(
                        f"Provider {self.configs[i].provider.value} failed ({exc}), "
                        f"falling back to {self.configs[i + 1].provider.value}..."
                    )
...
def _create_chained_client(config: AIConfig) -> ChainedAIClient:
    ...
    from xiaoyu.horizon.models import AI_PROVIDER_DEFAULTS
...
class XiaoyuAIClient(AIClient):
    """AIClient adapter that delegates to Xiaoyu's model selection system.

    Uses select_model() to resolve a model spec string (e.g. "openai:gpt-4o")
    to an OpenAIBase instance, then calls it with system+user messages.

    When json_mode=True, response_format={"type": "json_object"} is injected
    into the request for OpenAI-compatible providers. For other providers,
    a JSON-mode instruction is appended to the system prompt instead.
    """

    def __init__(self, model_spec: str, json_mode: bool = False):
        from xiaoyu.models.chat import select_model

        self._model_spec = model_spec
        self._json_mode = json_mode
        self._client = select_model(model_spec)
        self._provider_label = model_spec
