...
class ContentEnricher:
        ...
        async def _process(item: ContentItem) -> None:
            async with semaphore:
                try:
                    ...
                    logger.warning(f"Error enriching item {item.id}: {e}, falling back to translation")
                    await self._translate_item(item)

        logger.info(f"Enriching {len(items)} items with background knowledge...")
        coros = [_process(item) for item in items]
    ...
    async def _enrich_item(self, item: ContentItem) -> None:
        ...
        if result is None:
            ...
            logger.warning(f"Could not parse enrichment response for {item.id}, falling back to translation")
            await self._translate_item(item)
            return
