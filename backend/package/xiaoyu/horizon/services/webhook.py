...
from xiaoyu.horizon.ai.markdown_utils import clean_app_summary_markdown
from xiaoyu.horizon.models import ContentItem, WebhookConfig
...
class WebhookNotifier:
    ...
    def __init__(self, config: WebhookConfig, console=None):
        self.config = config
        # console parameter kept for backward compat; logging is used instead
        self.url = None
    ...
    def _validate_config(self) -> None:
        ...
        if not self.config.url_env:
            ...
            logger.warning(
                "Webhook enabled but 'url_env' is not set in config. "
                "No notification URL available, skipping."
            )
            return
        ...
        if raw_url is None:
            ...
            logger.warning(
                f"Webhook enabled but env var '{self.config.url_env}' is not set "
                f"in your environment. Skipping notification."
            )
            return
    ...
    async def notify(self, variables: dict) -> None:
        ...
        if not self.url:
            ...
            logger.warning(
                f"Webhook enabled but URL is empty — "
                f"env var '{self.config.url_env}' is not set. Skipping notification."
            )
            return
    ...
    def _handle_response_status(self, response: httpx.Response, safe_url: str) -> None:
        ...
        if 300 <= status < 400:
            ...
            logger.info(f"Webhook received redirect (status={status})")
            logger.warning(
    ...
    async def send_daily_summary(
        ...
        logger.info(f"🔔 Sending {lang.upper()} webhook notification...")
    ...
    async def send_failure(
        ...
        """
        logger.info("🔔 Sending webhook failure notification...")
        await self.notify(
            {
                "date": date,
                "language": "",
