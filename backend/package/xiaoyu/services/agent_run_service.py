...
from xiaoyu.agents.models import resolve_chat_model_spec
...
from xiaoyu.storage.postgres.models_business import Message, User
...
def _compact_run_event_envelope(envelope: dict) -> dict | None:
    ...
    if event_type == "metadata":
        return None
    if event_type == "custom" and isinstance(payload, dict) and payload.get("name") == "xiaoyu.agent_state":
...
async def get_active_run_by_thread(*, thread_id: str, current_uid: str, db: AsyncSession) -> dict:
    from sqlalchemy import select
    from xiaoyu.storage.postgres.models_business import AgentRun
