...
from xiaoyu.repositories.agent_run_repository import TERMINAL_RUN_STATUSES, AgentRunRepository
from xiaoyu.services.chat_service import stream_agent_chat, stream_agent_resume
...
def _map_chunk_to_run_event(chunk: dict) -> tuple[str, dict]:
    ...
    if status == "agent_state":
        return "custom", {"name": "xiaoyu.agent_state", "chunk": chunk, "agent_state": chunk.get("agent_state") or {}}
...
async def process_horizon_run(ctx, digest_id: str):
    """ARQ job: execute the Horizon news pipeline for a digest."""
    from xiaoyu.services.news_service import NewsService

    service = NewsService()
    await service.execute_run(ctx, digest_id)


class WorkerSettings:
    functions = [process_agent_run, process_horizon_run]
    max_tries = 2
    retry_jobs = True
    job_timeout = 3600
    keep_result = 60
