...
from xiaoyu.storage.postgres.models_business import Base as BusinessBase
from xiaoyu.storage.postgres.models_knowledge import Base as KnowledgeBase
import xiaoyu.storage.postgres.models_news  # noqa: F401  — register tables with BusinessBase metadata
from xiaoyu.utils import logger
from xiaoyu.utils.singleton import SingletonMeta
    ...
    async def ensure_business_schema(self):
        ...
        stmts = [
            ...
            CREATE TABLE IF NOT EXISTS news_schedules (
                ...
                trigger_time VARCHAR(5) DEFAULT '08:00',
                model_spec VARCHAR,
                language VARCHAR(5) DEFAULT 'zh',
                source_config JSONB DEFAULT '{}'::jsonb,
                ai_score_threshold FLOAT DEFAULT 5.0,
                ...
                CONSTRAINT uq_news_schedule_name_created_by UNIQUE (name, created_by)
            )
            """,
            "CREATE INDEX IF NOT EXISTS idx_news_schedules_created_by ON news_schedules(created_by)",
            """
            CREATE TABLE IF NOT EXISTS news_digests (
                id VARCHAR(64) PRIMARY KEY,
                schedule_id VARCHAR(64) REFERENCES news_schedules(id) ON DELETE SET NULL,
            ...
            """,
            "CREATE INDEX IF NOT EXISTS idx_news_digests_date ON news_digests(digest_date DESC)",
            "CREATE INDEX IF NOT EXISTS idx_news_digests_status ON news_digests(status)",
            "CREATE INDEX IF NOT EXISTS idx_news_digests_created_by ON news_digests(created_by)",
        ]
        async with self.async_engine.begin() as conn:
            for stmt in stmts:
                await conn.execute(text(stmt))
