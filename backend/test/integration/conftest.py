...
SANDBOX_CONTAINER_PREFIX = os.getenv("XIAOYU_SANDBOX_CONTAINER_PREFIX", "xiaoyu-sandbox")
...
def _docker_api_request(method: str, path: str) -> list[dict] | dict:
    cmd = [
        ...
        "-sS",
        "--unix-socket",
        os.getenv("XIAOYU_DOCKER_API_SOCKET", "/var/run/docker.sock"),
        "-X",
        method,
        f"{os.getenv('XIAOYU_DOCKER_API_BASE', 'http://localhost').rstrip('/')}{path}",
    ]
