...
def _load_context_module():
    context_path = Path(__file__).resolve().parents[3] / "package/xiaoyu/agents/context.py"
    previous_xiaoyu = sys.modules.get("xiaoyu")
    sys.modules["xiaoyu"] = types.SimpleNamespace(config=types.SimpleNamespace(default_model="test:model"))
    try:
        spec = importlib.util.spec_from_file_location("test_xiaoyu_agents_context", context_path)
        ...
        if previous_xiaoyu is None:
            sys.modules.pop("xiaoyu", None)
        else:
            sys.modules["xiaoyu"] = previous_xiaoyu
...
async def test_normalize_agent_context_config_expands_null_and_filters_explicit_lists(monkeypatch):
    ...
    monkeypatch.setitem(
        sys.modules,
        "xiaoyu.agents.toolkits.service",
