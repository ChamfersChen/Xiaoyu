...
from xiaoyu.agents.backends.composite import (
    CustomCompositeBackend,
    create_agent_composite_backend,
    create_agent_filesystem_middleware,
)
from xiaoyu.agents.backends.sandbox import resolve_virtual_path, sandbox_id_for_thread
from xiaoyu.agents.backends.sandbox.backend import ProvisionerSandboxBackend
from xiaoyu.agents.middlewares.skills import SkillsMiddleware
from xiaoyu.utils.paths import VIRTUAL_PATH_CONVERSATION_HISTORY, VIRTUAL_PATH_LARGE_TOOL_RESULTS
...
def test_create_agent_composite_backend_uses_prepared_readable_skills(monkeypatch):
    monkeypatch.setattr("xiaoyu.agents.backends.sandbox.backend.get_sandbox_provider", lambda: object())
...
def test_provider_uses_distinct_sandbox_scope_for_different_uid(monkeypatch) -> None:
    from xiaoyu.agents.backends.sandbox.provider import ProvisionerSandboxProvider
    ...
    monkeypatch.setattr("xiaoyu.agents.backends.sandbox.provider.load_user_agent_env", lambda uid: {"A": uid})
...
def test_provisioner_uses_file_and_skills_thread_ids(monkeypatch) -> None:
    ...
    monkeypatch.setattr("xiaoyu.agents.backends.sandbox.backend.get_sandbox_provider", lambda: FakeProvider())
    monkeypatch.setattr(
        "xiaoyu.agents.backends.sandbox.backend.sync_thread_readable_skills",
