...
import pytest
import xiaoyu.agents.middlewares.subagent_task as subagent_task_middleware
from langchain_core.messages import AIMessage, HumanMessage
from langgraph.prebuilt.tool_node import ToolRuntime
from langgraph.types import Command
from xiaoyu.agents.buildin.chatbot.state import merge_subagent_runs
from xiaoyu.agents.middlewares.subagent_task import XiaoyuSubAgentMiddleware
from xiaoyu.utils.subagent_thread_utils import make_child_thread_id
from xiaoyu.repositories.agent_repository import SUB_AGENT_BACKEND_ID
...
def _patch_subagent_run_tracking(monkeypatch):
    ...
    monkeypatch.setattr(XiaoyuSubAgentMiddleware, "_create_subagent_run", create_run)
    monkeypatch.setattr(XiaoyuSubAgentMiddleware, "_set_subagent_run_status", set_status)
...
async def test_create_task_middleware_loads_all_visible_subagents_when_empty(monkeypatch) -> None:
    ...
    assert isinstance(middleware, XiaoyuSubAgentMiddleware)
...
async def test_task_tool_rejects_unconfigured_subagent() -> None:
    middleware = XiaoyuSubAgentMiddleware(
...
async def test_task_tool_rejects_invalid_continuation_thread(monkeypatch) -> None:
    ...
    monkeypatch.setattr(XiaoyuSubAgentMiddleware, "_create_subagent_run", reject_continuation)
