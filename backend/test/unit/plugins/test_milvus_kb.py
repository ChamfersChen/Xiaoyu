...
if "knowledge_base" not in vars(xiaoyu):
    xiaoyu.knowledge_base = types.SimpleNamespace()

from xiaoyu.knowledge.implementations.milvus import (
...
async def test_insert_chunks_to_stores_inserts_current_batch(monkeypatch):
    ...
    monkeypatch.setattr("xiaoyu.knowledge.implementations.milvus.KnowledgeChunkRepository", FakeChunkRepo)
    kb = MilvusKB.__new__(MilvusKB)
...
async def test_update_content_uses_streaming_chunk_store(monkeypatch):
    ...
    monkeypatch.setattr("xiaoyu.knowledge.implementations.milvus.Parser.aparse", parse_file)
