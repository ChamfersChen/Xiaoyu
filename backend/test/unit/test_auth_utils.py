...
import jwt
import pytest
from xiaoyu.utils.datetime_utils import utc_now

from xiaoyu.utils.auth_utils import JWT_ALGORITHM, JWT_AUDIENCE, AuthUtils
...
def test_access_token_requires_configured_secret_in_production(monkeypatch):
    monkeypatch.setenv("XIAOYU_ENV", "production")
    monkeypatch.delenv("JWT_SECRET_KEY", raising=False)
    monkeypatch.setenv("XIAOYU_INSTANCE_ID", "pytest-instance")
...
def test_access_token_rejects_public_default_secret_in_production(monkeypatch):
    monkeypatch.setenv("XIAOYU_ENV", "production")
    monkeypatch.setenv("JWT_SECRET_KEY", "xiaoyu_know_secure_key")
    monkeypatch.setenv("XIAOYU_INSTANCE_ID", "pytest-instance")
...
def test_access_token_auto_generates_dev_instance_id(monkeypatch):
    monkeypatch.setenv("XIAOYU_ENV", "development")
    monkeypatch.setenv("JWT_SECRET_KEY", "test-secret-key-with-enough-randomness")
    monkeypatch.delenv("XIAOYU_INSTANCE_ID", raising=False)

    token = AuthUtils.create_access_token({"sub": "1"})

    assert AuthUtils.verify_access_token(token)["iss"].startswith("xiaoyu-know:instance-")
    assert os.environ["XIAOYU_INSTANCE_ID"].startswith("instance-")
...
def test_access_token_requires_instance_id_in_production(monkeypatch):
    ...
    with pytest.raises(ValueError, match="XIAOYU_INSTANCE_ID"):
        AuthUtils.create_access_token({"sub": "1"})
...
def test_verify_access_token_rejects_wrong_issuer(monkeypatch):
    ...
    monkeypatch.setenv("XIAOYU_INSTANCE_ID", "pytest-instance")
    token = jwt.encode(
        {"sub": "1", "exp": utc_now() + timedelta(minutes=5), "iss": "xiaoyu-know:other", "aud": JWT_AUDIENCE},
...
def test_verify_access_token_rejects_wrong_audience(monkeypatch):
    ...
    monkeypatch.setenv("XIAOYU_INSTANCE_ID", "pytest-instance")
    token = jwt.encode(
        {"sub": "1", "exp": utc_now() + timedelta(minutes=5), "iss": "xiaoyu-know:pytest-instance", "aud": "other-api"},
        "test-secret-key-with-enough-randomness",
        algorithm=JWT_ALGORITHM,
    )
