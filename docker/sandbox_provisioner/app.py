...
class LocalContainerProvisionerBackend:
    def __init__(self):
        ...
        self._container_prefix = os.getenv("DOCKER_SANDBOX_PREFIX", "xiaoyu-sandbox")
        ...
        with self._lock:
            ...
            run_kwargs = {
                ...
                "labels": {
                    ...
                    "uid": safe_uid,
                    "managed-by": "xiaoyu-sandbox-provisioner",
                },
                "volumes": {
    ...
    def list(self) -> list[SandboxRecord]:
        containers = self._client.containers.list(
            all=True, filters={"label": ["app=xiaoyu-sandbox", "managed-by=xiaoyu-sandbox-provisioner"]}
        )
...
class KubernetesProvisionerBackend:
    def __init__(self):
        ...
        self._lock = threading.Lock()
        self._namespace = os.getenv("K8S_NAMESPACE", "xiaoyu-know")
        self._sandbox_image = os.getenv(
            "SANDBOX_IMAGE",
            "enterprise-public-cn-beijing.cr.volces.com/vefaas-public/all-in-one-sandbox:latest",
        )
        self._skill_pvc = os.getenv("SKILLS_PVC", "xiaoyu-skills")
        self._thread_pvc = os.getenv("THREAD_PVC", "xiaoyu-thread")
        ...
        *,
        ...
        return self._client.V1Pod(
            metadata=self._client.V1ObjectMeta(
                name=pod_name,
                labels={"app": "xiaoyu-sandbox", "sandbox-id": sandbox_id},
                annotations={
                    "thread-id": thread_id,
    ...
    def list(self) -> list[SandboxRecord]:
        ...
        try:
            pod_list = self._core_api.list_namespaced_pod(
                namespace=self._namespace,
                label_selector="app=xiaoyu-sandbox",
            )
        except ApiException:
            return []
...
app = FastAPI(title="Xiaoyu Sandbox Provisioner", lifespan=lifespan)
