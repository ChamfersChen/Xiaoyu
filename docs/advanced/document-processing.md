...
Xiaoyu 支持多种文档格式的智能解析，从简单的文本文件到复杂的 PDF 文档，都能自动提取内容并转换为可检索的格式。
...
1. 配置 `XIAOYU_URL_WHITELIST` 环境变量启用白名单机制
2. 系统自动将 HTML 转换为 Markdown
3. 内置去重机制，避免重复抓取

::: tip URL 白名单配置
示例：`XIAOYU_URL_WHITELIST=github.com,*.wikipedia.org,docs.python.org`
:::
