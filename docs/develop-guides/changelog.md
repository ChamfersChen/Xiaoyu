...
- 发布新闻热点检索与摘要编辑功能技术设计文档（`docs/vibe/2026-06-13-news-summary-feature.md`），方案基于 Horizon 受控分销 + XiaoyuAIClient 适配器 + JSONB 单表存储 + asyncio 定时调度 + MinIO 文件归档 + Webhook 投递。

- **新闻热点后端实现**：将 Horizon 受控分销集成到 `xiaoyu/horizon/`，移除 StorageManager/EmailManager/Email/GitHubPages 依赖，改用 Xiaoyu 的 LLM 模型体系（`XiaoyuAIClient` 适配 `select_model`）；新增 `models_news.py`（NewsSchedule + NewsDigest ORM）、`news_repository.py`、`news_service.py`（管道编排 + MinIO 上传 + Webhook 投递）、`news_router.py`（REST API：摘要 CRUD、触发、编辑、重生成、下载、Webhook 重试、调度管理）；ARQ Worker 注册 `process_horizon_run` 任务；lifespan 启动新闻定时调度后台协程；`rich` 依赖全部替换为 `logging`；数据库迁移语句加入 `ensure_business_schema()`。

- 发布版本号更新至 `0.1.0`，同步 package、Docker 镜像标签与快速开始分支引用。
...
- Agent run SSE 新增 `verbose=false` 精简模式：默认仍返回完整事件载荷；精简模式仅在 SSE 输出前重建最小 payload，跳过 `metadata` 和空 `xiaoyu.agent_state`，将同一 data 内的 `request_id` 外提为单个字段，移除 chunk 中重复的 `meta`、`metadata`、`thread_id`、`response`、空 `namespace` 和图片 base64 等调试字段，保留消息增量、工具调用、工具结果、非空 Agent state、终止状态和 SSE 游标，前端订阅默认使用精简模式。
...
- 加固 JWT 鉴权安全：移除历史默认密钥回退，初始化脚本支持生成并持久化 `JWT_SECRET_KEY` 与 `XIAOYU_INSTANCE_ID`，签发和验证令牌时校验 `iss/aud`，并拒绝已删除或登录锁定用户继续使用旧令牌访问系统。
...
- 将 `xiaoyu` 从 uv workspace 成员调整为 `backend/package` 下可独立构建的本地 Python 包，backend 通过 path dependency 以已安装包形式发现依赖
...
### 新增
- 重构后端代码 src -> backend/package/xiaoyu
...
- 补充 Langfuse 集成方案文档：明确采用“云端优先、先 tracing 后 feedback”的接入路径，并约定 Xiaoyu 的 `user/thread` 到 Langfuse `user_id/session_id` 的映射关系
...
- 重构 MCP 相关配置，使用数据库来控制 [#469](https://github.com/xerrors/Xiaoyu/pull/469)
- 使用 docling 解析 office 文件（docx/xlsx/pptx）
- 优化后端的依赖，减少镜像体积 [#428](https://github.com/xerrors/Xiaoyu/issues/428)
...
- 修复部分场景下获取工具列表报错 [#470](https://github.com/xerrors/Xiaoyu/pull/470)
- 修改方法备注信息 [#478](https://github.com/xerrors/Xiaoyu/pull/478)
- 修复多次 human-in-the-loop 的渲染解析问题 [#453](https://github.com/xerrors/Xiaoyu/issues/453) [#475](https://github.com/xerrors/Xiaoyu/pull/475)
...
### 新增
- 新增对于上传附件的智能体中间件，详见[文档](https://xerrors.github.io/Xiaoyu/advanced/agents-config.html#%E6%96%87%E4%BB%B6%E4%B8%8A%E4%BC%A0%E4%B8%AD%E9%97%B4%E4%BB%B6)
- 新增多模态模型支持（当前仅支持图片），详见[文档](https://xerrors.github.io/Xiaoyu/advanced/agents-config.html#%E5%A4%9A%E6%A8%A1%E6%80%81%E5%9B%BE%E7%89%87%E6%94%AF%E6%8C%81)
- 新建 DeepAgents 智能体（深度分析智能体），支持 todo，files 等渲染，支持文件的下载。
- 新增基于知识库文件生成思维导图功能（[#335](https://github.com/xerrors/Xiaoyu/pull/335#issuecomment-3530976425)）
- 新增基于知识库文件生成示例问题功能（[#335](https://github.com/xerrors/Xiaoyu/pull/335#issuecomment-3530976425)）
- 新增知识库支持文件夹/压缩包上传的功能（[#335](https://github.com/xerrors/Xiaoyu/pull/335#issuecomment-3530976425)）
- 新增自定义模型支持、新增 dashscope rerank/embeddings 模型的支持
- 新增文档解析的图片支持，已支持 MinerU Officical、Docs、Markdown Zip格式
- 新增暗色模式支持并调整整体 UI（[#343](https://github.com/xerrors/Xiaoyu/pull/343)）
- 新增知识库评估功能，支持导入评估基准或者自动构建评估基准（目前仅支持Milvus类型知识库）详见[文档](https://xerrors.github.io/Xiaoyu/intro/evaluation.html)
- 新增同名文件处理逻辑：遇到同名文件则在上传区域提示，是否删除旧文件
- 新增生产环境部署脚本，固定 python 依赖版本，提升部署稳定性
- 优化图谱可视化方式，统一图谱数据结构，统一使用基于 G6 的可视化方式，同时支持上传带属性的图谱文件，详见[文档](https://xerrors.github.io/Xiaoyu/intro/knowledge-base.html#_1-%E4%BB%A5%E4%B8%89%E5%85%83%E7%BB%84%E5%BD%A2%E5%BC%8F%E5%AF%BC%E5%85%A5)
