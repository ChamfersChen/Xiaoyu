# Xiaoyu Initialization Script for PowerShell
# This script helps set up the environment for the Xiaoyu project
...
function Ensure-JwtEnv {
    if ((Test-EnvValue "JWT_SECRET_KEY") -and (Test-EnvValue "XIAOYU_INSTANCE_ID")) {
        return
    }
    ...
    $XIAOYU_INSTANCE_ID = Read-Host "Please enter your XIAOYU_INSTANCE_ID (press Enter to auto-generate)"
    if ([string]::IsNullOrEmpty($XIAOYU_INSTANCE_ID)) {
        $XIAOYU_INSTANCE_ID = "instance-$(New-RandomHex 8)"
        Write-Host "Generated XIAOYU_INSTANCE_ID and saved it to .env." -ForegroundColor Green
    }
...
XIAOYU_INSTANCE_ID=$XIAOYU_INSTANCE_ID
"@ | Add-Content -Path ".env" -Encoding UTF8
}

Write-Host "🚀 Initializing Xiaoyu project..." -ForegroundColor Cyan
...
"@
    ...
    Remove-Variable -Name "XIAOYU_INSTANCE_ID" -ErrorAction SilentlyContinue
}
