...
# Xiaoyu Initialization Script for Bash/Linux/macOS
...
ensure_jwt_env() {
    if grep -Eq '^JWT_SECRET_KEY=.+' .env && grep -Eq '^XIAOYU_INSTANCE_ID=.+' .env; then
        return
    fi
    ...
    read -p "Please enter your XIAOYU_INSTANCE_ID (press Enter to auto-generate): " XIAOYU_INSTANCE_ID
    if [ -z "$XIAOYU_INSTANCE_ID" ]; then
        XIAOYU_INSTANCE_ID="instance-$(generate_hex 8)"
        echo "Generated XIAOYU_INSTANCE_ID and saved it to .env."
    fi
...
echo "🚀 Initializing Xiaoyu project..."
echo "=================================="
